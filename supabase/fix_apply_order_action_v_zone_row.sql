-- ═══════════════════════════════════════════════════════════════════
-- HOTFIX 2026-05-20: bug "record v_zone_row is not assigned yet"
--
-- Problema: PL/pgSQL no puede inferir el tipo de v_zone_row.zona en el
-- CASE final cuando v_zone_row nunca fue asignado (cualquier acción que
-- no sea change_address). Esto rompió 'remind_payment' (y rompería
-- verify/cancel/etc también).
--
-- Fix: reemplazar el RECORD v_zone_row por 3 variables escalares.
...
DECLARE
    ...
    -- change_address scratch (escalares en vez de RECORD)
    v_old_address    text;
    v_old_fee        integer;
    v_old_total      integer;
...
BEGIN
    ...
    ELSIF p_action = 'change_address' THEN
        ...
        -- Usar variables escalares en vez de un RECORD (PL/pgSQL evita la
        -- inferencia de tipo problemática del CASE final).
        ...
        IF v_zone_zona IS NULL THEN
            -- Sin match: usar FUERA como fallback.
...
-- ═══════════════════════════════════════════════════════════════════
-- 1. Probar remind_payment (lo que estaba roto):
--    SELECT apply_order_action(
--      (SELECT order_id FROM orders WHERE payment_status='pending'
--       ORDER BY created_at DESC LIMIT 1),
--      'remind_payment', 'manu_test', 'crm'
--    );
--    → ok:true, reply_text con el recordatorio.
--
-- 2. Probar cancel (también afectado por el bug):
--    SELECT apply_order_action(<pedido pending>, 'cancel', 'manu_test', 'crm');
--    → ok:true, reply_text con mensaje cálido, INSERT en chat_memory.
--
-- 3. Probar verify (sanity, debería seguir funcionando):
--    SELECT apply_order_action(<pedido pending>, 'verify', 'manu_test', 'crm');
